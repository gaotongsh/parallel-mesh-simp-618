...
struct {
    double time;
    char* note;
} watchSamples[N_SAMPLES];

static int sampleCount = 0;

static int clockID = 0;

static char* clockName = "Default Clock";
...
void watchStart(char* name) {
    ...
    if (name)
        ...
        clockName = "Default Clock";
    sampleCount = 0;
    startTime = currentSeconds();
}

void watchTake(char* sampleName) {
    assert(sampleCount < N_SAMPLES);
    watchSamples[sampleCount].time = currentSeconds();
    watchSamples[sampleCount].note = sampleName;
    sampleCount++;
}

void watchReport() {
    fprintf(stderr, "=== Timing report of watch '%s - %d'. ===\n", clockName, clockID);
    double lastsample = startTime;
    int i;
    for (i = 0; i < sampleCount; ++i) {
        fprintf(stderr, "[%3d] %-30s - %-15.2f - %-15.2f\n",
                i, watchSamples[i].note,
               (watchSamples[i].time - startTime) * 1000,
               (watchSamples[i].time - lastsample) * 1000
               );
        lastsample = watchSamples[i].time;
    }
    fputs("=== END OF STOPWATCH REPORT ===\n", stderr);
}
